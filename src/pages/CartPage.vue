...
<section class="h-full w-full flex flex-col">
  <div class="container grid grid-cols-2 gap-8 py-5 max-lg:grid-cols-1">
    <div class="flex flex-col justify-center gap-5">
      <CartProductItem v-if="store.filteredUniqueCartItems.length > 0" :products="store.filteredUniqueCartItems" />
      <AppButton
          v-else
          label="Get product"
    ...
    <div class="w-full  max-h-[200px] rounded-2xl flex bg-white flex-col justify-between p-5 gap-5 shadow-2xl buy">
      <h2 class="font-bold text-3xl text-gray-900 sm:text-left sm:text-5xl self-center">
